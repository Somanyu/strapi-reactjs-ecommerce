    ...
    <Link to={`/product/${item?.id}`} className="group relative block overflow-hidden">
      {
        ...
        <span className="bg-blue-200 font-secondary z-10 text-sm font-medium inline-flex items-center text-blue-800 p-1 leading-none rounded-md px-2 dark:bg-blue-900 dark:text-blue-200 absolute -translate-y-1/2 -translate-x-1/2 right-auto top-[1.1rem] left-[3.7rem]">
          <BsStars />
          New Season
        </span>
      }
      ...
      <div className="relative bg-white pt-3">
        <h3 className="text-lg font-semibold font-primary text-gray-700">{item?.attributes.title}</h3>
        <p className="mt-1.5 font-secondary tracking-wide text-gray-900">${item?.attributes.price}</p>
      </div>
    </Link>
  )
}
